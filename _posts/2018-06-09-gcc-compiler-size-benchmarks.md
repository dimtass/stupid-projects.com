...
[https://github.com/dimtass/stm32f103-usb-periph-expander](https://github.com/dimtass/stm32f103-usb-periph-expander)
...
Also it seems that when no optimizations are applied with `-O0`then the `-flto`instead of doing size optimization, actually created a larger binary. I have no explain for that, but anyways it doesn't really matter, because there's no point in using `-flto`at all in such cases.
